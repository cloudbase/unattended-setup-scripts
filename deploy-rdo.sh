...
if [ $# -ne 6 ]; then
    echo "Usage: $0 <esxi_user> <esxi_host> <datastore> <rdo_name> <esxi_public_switch> <esxi_public_vnic>"
    exit 1
fi
...
ssh $ESXI_USER@$ESXI_HOST $ESXI_BASEDIR/deploy-rdo-vms.sh $DATASTORE $RDO_NAME $ESXI_PUBLIC_SWITCH $ESXI_PUBLIC_VNIC $RDO_VM_IPS_FILE
